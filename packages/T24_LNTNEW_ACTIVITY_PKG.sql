    ...
    BEGIN
        ...
        IF V_JOIN_KEY_LIST.COUNT > 0 THEN
            ...
            SELECT
                ...
                AAC.MSB_LN_PURPOSE AS PURCOD,
                GREATEST(
                    CASE WHEN PRE.WINDOW_ID  IS NOT NULL THEN PRE.WINDOW_ID  END, 
                    CASE WHEN AIT.WINDOW_ID  IS NOT NULL THEN AIT.WINDOW_ID  END, 
                    CASE WHEN ASCC.WINDOW_ID IS NOT NULL THEN ASCC.WINDOW_ID END, 
                    CASE WHEN AAC.WINDOW_ID  IS NOT NULL THEN AAC.WINDOW_ID  END, 
                    CASE WHEN ATA.WINDOW_ID  IS NOT NULL THEN ATA.WINDOW_ID  END
                ) AS WINDOW_ID,
                GREATEST(
                    CASE WHEN PRE.COMMIT_TS  IS NOT NULL THEN PRE.COMMIT_TS  END,
                    CASE WHEN AIT.COMMIT_TS  IS NOT NULL THEN AIT.COMMIT_TS  END,
                    CASE WHEN ASCC.COMMIT_TS IS NOT NULL THEN ASCC.COMMIT_TS END,
                    CASE WHEN AAC.COMMIT_TS  IS NOT NULL THEN AAC.COMMIT_TS  END,
                    CASE WHEN ATA.COMMIT_TS  IS NOT NULL THEN ATA.COMMIT_TS  END
                ) AS COMMIT_TS
                GREATEST(
                    CASE WHEN PRE.REPLICAT_TS  IS NOT NULL THEN PRE.REPLICAT_TS  END,
                    CASE WHEN AIT.REPLICAT_TS  IS NOT NULL THEN AIT.REPLICAT_TS  END,
                    CASE WHEN ASCC.REPLICAT_TS IS NOT NULL THEN ASCC.REPLICAT_TS END,
                    CASE WHEN AAC.REPLICAT_TS  IS NOT NULL THEN AAC.REPLICAT_TS  END,
                    CASE WHEN ATA.REPLICAT_TS  IS NOT NULL THEN ATA.REPLICAT_TS  END
                ) AS REPLICAT_TS,
                GREATEST(
                    CASE WHEN PRE.MAPPED_TS  IS NOT NULL THEN PRE.MAPPED_TS  END, 
                    CASE WHEN AIT.MAPPED_TS  IS NOT NULL THEN AIT.MAPPED_TS  END, 
                    CASE WHEN ASCC.MAPPED_TS IS NOT NULL THEN ASCC.MAPPED_TS END, 
                    CASE WHEN AAC.MAPPED_TS  IS NOT NULL THEN AAC.MAPPED_TS  END, 
                    CASE WHEN ATA.MAPPED_TS  IS NOT NULL THEN ATA.MAPPED_TS  END
                ) AS MAPPED_TS,               
                'ARR_AIT_ASC_AAC_ATA'
            FROM PRECOMPUTED PRE
            LEFT JOIN ARC_AGGREGATED ARC ON ARC.ARRANGEMENT = PRE.ARR_RECID
            INNER JOIN ATA_AGGREGATED ATA ON ATA.ID_COMP_1 = PRE.ARR_RECID
            INNER JOIN ASC_AGGREGATED ASCC ON ASCC.ID_COMP_1 = PRE.ARR_RECID
            INNER JOIN AAC_AGGREGATED AAC ON AAC.ID_COMP_1 = PRE.ARR_RECID
