CREATE OR REPLACE PACKAGE T24RAWOGG.T24_DDMEMO_ACTIVITY_PKG IS
    ...
    PROCEDURE GEN_FROM_ACC_ARR_ECB_LMT_PROC;
...
---------------------------------------------------------------------------
-- GEN_FROM_ACC_ARR_ECB_LMT_PROC
---------------------------------------------------------------------------
    PROCEDURE GEN_FROM_ACC_ARR_ECB_LMT_PROC IS 
    ...
    BEGIN
        ...
        FROM T24_DDMEMO_ACTIVITY_ACC_ARR_ECB_LMT CDC
        WHERE EXISTS (
            SELECT 1
        ...
        IF V_JOIN_KEY_LIST.COUNT > 0 THEN
            ...
            DELETE FROM T24_DDMEMO_ACTIVITY_ACC_ARR_ECB_LMT CDC
            WHERE EXISTS (
                SELECT 1
            ...
            PRECOMPUTED AS(
                ...
                LEFT JOIN  F_PST_MAPPED      PST ON PST.RECID          = T24_UTILS_PKG.GET_STR_VAL_BY_POS_FUNC(ACC.POSTING_RESTRICT, '#1:')  
            ...
            SELECT
                ...
                NVL(
                    ...
                    TO_NUMBER(TO_CHAR(TO_DATE(V_TODAY, 'YYYYMMDD'), 'DDMMYY'))
                ) AS DLA6,
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            RAISE;
    END GEN_FROM_ACC_ARR_ECB_LMT_PROC;
