...
CREATE OR REPLACE PACKAGE BODY T24RAWOGG.T24_LNMEMO_ACTIVITY_PKG IS
    ...
    PROCEDURE GEN_FROM_ACC_PROC IS
        ...
        V_TODAY          VARCHAR2(8);
    BEGIN
        DELETE /*+ PARALLEL(CDC, 8) */ FROM T24_LNMEMO_ACTIVITY_ACC CDC
        WHERE EXISTS (
            SELECT 1
        ...
        WITH PRECOMPUTED AS (
            ...
            INNER JOIN FMSB_ACC_MAPPED ACC ON ACC.WINDOW_ID      = V.COLUMN_VALUE AND ACC.OP_TYPE <> 'D'
            INNER JOIN FMSB_ECB_MAPPED ECB ON ECB.RECID          = ACC.RECID
            INNER JOIN FMSB_ARR_LNMEMO ARR ON ARR.LINKED_APPL_ID = ACC.RECID
            LEFT  JOIN FMSB_LMT_MAPPED LMT ON LMT.RECID          = ACC.LIMIT_KEY  AND LMT.OP_TYPE <> 'D'
        ),
        AGGREGATED AS (
            SELECT
                BIL.ARRANGEMENT_ID,
    ...
    PROCEDURE GEN_FROM_ARR_PROC IS
        ...
        V_TODAY          VARCHAR2(8);
    BEGIN
        DELETE /*+ PARALLEL(CDC, 8) */ FROM T24_LNMEMO_ACTIVITY_ARR CDC
        WHERE EXISTS (
            SELECT 1
        ...
        WITH PRECOMPUTED AS (
            ...
            FROM TABLE(V_WINDOW_ID_LIST) V
            INNER JOIN FMSB_ARR_LNMEMO ARR ON ARR.WINDOW_ID = V.COLUMN_VALUE     AND ARR.OP_TYPE <> 'D'
    ...
    PROCEDURE GEN_FROM_BIL_PROC IS
        ...
        V_TODAY          VARCHAR2(8);
    BEGIN
        DELETE /*+ PARALLEL(CDC, 8) */ FROM T24_LNMEMO_ACTIVITY_BIL CDC
        WHERE EXISTS (
            SELECT 1
            FROM FMSB_BIL_LNMEMO BIL
        ...
        WITH PRECOMPUTED AS (
            ...
            FROM TABLE(V_WINDOW_ID_LIST) V
            INNER JOIN FMSB_BIL_LNMEMO BIL ON BIL.WINDOW_ID = V.COLUMN_VALUE     AND BIL.OP_TYPE <> 'D'
            INNER JOIN FMSB_ARR_LNMEMO ARR ON ARR.RECID     = BIL.ARRANGEMENT_ID AND ARR.OP_TYPE <> 'D'
            INNER JOIN FMSB_ACC_MAPPED ACC ON ACC.RECID     = ARR.LINKED_APPL_ID
            INNER JOIN FMSB_ECB_MAPPED ECB ON ECB.RECID     = ACC.RECID
            LEFT  JOIN FMSB_LMT_MAPPED LMT ON LMT.RECID     = ACC.LIMIT_KEY      AND LMT.OP_TYPE <> 'D'
        ),
    ...
    PROCEDURE GEN_FROM_ECB_PROC IS
        ...
        V_TODAY          VARCHAR2(8);
    BEGIN
        DELETE /*+ PARALLEL(CDC, 8) */ FROM T24_LNMEMO_ACTIVITY_ECB CDC
        WHERE EXISTS (
            SELECT 1
        ...
        WITH PRECOMPUTED AS (
            ...
            FROM TABLE(V_WINDOW_ID_LIST) V
            INNER JOIN FMSB_ECB_MAPPED ECB ON ECB.WINDOW_ID      = V.COLUMN_VALUE AND ECB.OP_TYPE <> 'D'
            INNER JOIN FMSB_ACC_MAPPED ACC ON ACC.RECID          = ECB.RECID
            INNER JOIN FMSB_ARR_LNMEMO ARR ON ARR.LINKED_APPL_ID = ECB.RECID
            LEFT  JOIN FMSB_LMT_MAPPED LMT ON LMT.RECID          = ACC.LIMIT_KEY  AND LMT.OP_TYPE <> 'D'
        ),
        AGGREGATED AS (
            SELECT
